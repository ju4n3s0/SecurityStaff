...
class ThreatDatabase:
    ...
    def save_manual_analysis(self, analysis: dict) -> int:
        """
        Guarda un análisis manual (desde el formulario web) sin gmail_message_id.

        Args:
            analysis: Diccionario resultado de MessageResult.to_dict()

        Returns:
            ID de la fila insertada, o -1 si hubo error
        """
        import uuid
        try:
            cursor = self.conn.cursor()
            ...
            # Synthetic unique ID for non-Gmail messages
            synthetic_id = f"manual-{uuid.uuid4().hex[:16]}"
            ...
            self.conn.commit()
            threat_id = cursor.lastrowid
            logger.info(f"Análisis manual guardado con ID: {threat_id}")
            return threat_id

        except Exception as e:
            logger.error(f"Error guardando análisis manual: {e}")
            return -1
